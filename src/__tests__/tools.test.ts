...
describe("get_specification tool", () => {
  it("returns maths spec with topics", () => {
    const result = handleGetSpecification({ board: "AQA", subject: "Mathematics", level: "GCSE" });
    expect(result.code).toBe("8300");
    expect(result.topics.length).toBeGreaterThanOrEqual(5);
  });

  it("returns not-found for missing subject", () => {
    const result = handleGetSpecification({ board: "AQA", subject: "Basket Weaving", level: "GCSE" });
...
describe("get_paper_structure tool", () => {
  it("returns 6 papers for AQA Maths", () => {
    const result = handleGetPaperStructure({ board: "AQA", subject: "Mathematics", level: "GCSE" });
    expect(result.papers.length).toBe(6);
  });

  it("filters to single paper by id", () => {
    const result = handleGetPaperStructure({ board: "AQA", subject: "Mathematics", level: "GCSE", paper: "8300/1H" });
